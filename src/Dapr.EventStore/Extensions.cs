...
public static class Extensions
{
    ...
    public static string JsonQuery(string streamName)
        => "{" +
                "\"filter\": {" +
    ...
    public static async IAsyncEnumerable<EventData> LoadAsyncStreamNameQueryAsync(
        this DaprClient client, string storeName,
        string streamName, long version, Dictionary<string, string> meta)
    {
        var query = JsonQuery(streamName);
        var r = await client.QueryStateAsync<EventData>(storeName, query, metadata: meta);
        foreach (var item in r.Results
                                .Where(x => x.Data.Version >= version)
                                .OrderBy(x => x.Data.Version))
        ...
        string streamName, string streamHeadKey, StreamHead head, string headetag, Dictionary<string, string> meta, EventData[] versionedEvents)
    {
        ...
        var headReq = new StateTransactionRequest(streamHeadKey, JsonSerializer.SerializeToUtf8Bytes(head), Client.StateOperationType.Upsert,
            metadata: meta,
            etag: string.IsNullOrWhiteSpace(headetag) ? null : headetag);
    ...
    public static async Task StateTransactionWithoutVesionSufixAsync(this DaprClient client,
        string storeName,
        string streamName, string streamHeadKey, StreamHead head, string headetag, Dictionary<string, string> meta, EventData[] versionedEvents)
    {
        var eventsReq = versionedEvents.Select(x => new StateTransactionRequest($"{streamName}|{x.EventId}", JsonSerializer.SerializeToUtf8Bytes(x), StateOperationType.Upsert, metadata: meta));
        var headReq = new StateTransactionRequest(streamHeadKey, JsonSerializer.SerializeToUtf8Bytes(head),
            Client.StateOperationType.Upsert, metadata: meta,
            etag: string.IsNullOrWhiteSpace(headetag) ? null : headetag);
        var reqs = new List<StateTransactionRequest>();
        reqs.AddRange(eventsReq);
        reqs.Add(headReq);
